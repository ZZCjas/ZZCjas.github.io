...
string so[7]={"","石斧步兵","持矛步兵","铁甲步兵","步枪手","坦克","海陆空作战部队"};
struct nation
{
	string name;
	long long army;
...
nation x[10000001];
	...
	void win()
	{
		for(int i=1;i<=n;i++)
		{
			if(x[i].alive)
			{
				cout<<x[i].name<<"称霸了世界!\n";
				cout<<x[i].name<<"数据:\n";
				cout<<"武力值:"<<x[i].army<<endl;
				cout<<"科技等级:"<<l[x[i].level]<<endl;
				system("pause");
				exit(0); 
			}
		}
	...
	//	cout<<cnt<<endl;
		...
		else if(b==3)
		{
			...
			int t=rand()%10+30;
			cout<<x[a].name<<"制造了"<<t*5<<"个"<<so[x[a].level]<<",军队战斗力增加"<<(x[a].level*x[a].level)*t<<endl;
				...
				if(x[a].level==6)
				{
					if(x[b].level==6)
					{
						...
						else
						{
							...
							{
								cout<<">>>";
								cout<<x[a].name<<"将"<<x[b].name<<"用核弹炸成了一片废墟,"<<x[b].name<<"灭亡了\n";
								if(x[b].level>=5)
								{
									nuclear.erase(b);
								}
							...
							{
								cout<<">>>";
								cout<<x[a].name<<"将"<<x[b].name<<"用核弹炸成了一片废墟,"<<x[b].name<<"文明降级\n";
								if(x[b].level==5)
								{
									nuclear.erase(b);
								}
		...
		for(int i=1;i<=n;i++)
		{
			...
			{
				cout<<">>>"<<x[i].name<<"的军队人口被消耗殆尽,"<<x[i].name<<"灭亡了\n";
				cnt--;
				Sleep(1000);
				x[i].alive=0;
				if(x[i].level>=5)
...
int main()
{
		...
		system("color b");
		cout<<"国家模拟器,Copyright 2023 ZZCjas\n";
		cout<<"国家数:";
		cin>>n;
		cout<<"各个国家的名字:\n";
		for(int i=1;i<=n;i++)
