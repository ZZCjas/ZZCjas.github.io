...
class NBodyApp:
    def __init__(self, root):
        ...
        self.max_total_frags = tk.IntVar(value=6)    # 全局碎片上限
        self.similar_thresh = 0.4      # 近质量碎裂阈值
        self.min_frag_mass   = 0.05    # 碎片最小质量（增大以减少数量）
        self.small_frag_scale_limit = 5  # 小体粉碎时最多碎片数（缩小）
    ...
    def load_example_three(self):
        ...
        self.sim.add_body( -1.0,  10.0,  0.0, 0, 2.0, 1.5, is_fragment=False)
        self.sim.add_body(  15.0, 0.0, 0.0,  -0.83333, m, d, is_fragment=False)
        self.status.set("示例：三体运动。(最好把边界半边长调到最大)")
        self._draw()
    ...
    def load_example_slingshot(self):
        ...
        start_x = 3.4
        start_y = -35.0
        vx_probe = 1.1   # 初始速度，靠得太慢会被捕获，太快则偏转太小
        vy_probe = 9.5
    ...
    def resolve_collision(self, idx_a, idx_b):
        ...
        if mass_ratio <= absorb_thresh:
            # 使用体积相加来计算合并后的直径，避免因直接按质量开方导致尺寸异常
